...
//      srun --reservation=fri --partition=gpu --gpus=1 ./heat4 8192
...
__global__ void heatStep(float* surfaceOut, float* surfaceIn, int width, int height) {
    ...
    // Load center
    tile[ty * BLOCK_SIZE + tx] = surfaceIn[gy * width + gx];

    // Halo loading
    if (lx == 0)         
        tile[ty * BLOCK_SIZE + 0] = surfaceIn[gy * width + (gx - 1)];
    if (lx == BLOCK_SIZE-1)   
        tile[ty * BLOCK_SIZE + BLOCK_SIZE+1] = surfaceIn[gy * width + (gx + 1)];
    if (ly == 0)
        tile[0 * BLOCK_SIZE + tx] = surfaceIn[(gy - 1) * width + gx];
    if (ly == BLOCK_SIZE-1)
       tile[(BLOCK_SIZE+1)*BLOCK_SIZE + tx] = surfaceIn[(gy + 1) * width + gx];

    __syncthreads();

    surfaceOut[idx] = 0.25 * (
        tile[ty * BLOCK_SIZE + tx - 1] + tile[ty * BLOCK_SIZE + tx + 1] +
        tile[(ty - 1) * BLOCK_SIZE + tx] + tile[(ty + 1) * BLOCK_SIZE + tx]
    );
}
...
int main(int argc, char *argv[]) {
	...
	for (int i = 0; i < MAXITERS; i++) {
		heatStep<<<grid, block, localMemSize>>>(d_surfaceNew, d_surface, N, N);
